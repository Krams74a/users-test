...
import {getGroupProfile} from "../../../../reducers/groupProfile-reducer";

const GroupAvatarSettings = (props) => {
    ...
    const onMainPhotoSelected = (event) => {
        if (event.target.files[0].size > 800000) {
            ...
            if (event.target.files.length) {
                props.uploadGroupAvatar(event.target.files[0], props.groupProfileInfo.groupName)
                    .then(response => {
                        if (response.status === 200) {
                            setSuccess(response.message)
                            props.getGroupProfile(props.groupProfileInfo.groupName)
                        } else {
    ...
    const onMainPhotoDeleted = () => {
        setSuccess(null)
        props.deleteGroupAvatar(props.groupProfileInfo.groupName)
            .then(response => {
                if (response.status === 200) {
                    setSuccess(response.message)
                    props.getGroupProfile(props.groupProfileInfo.groupName)
    ...
    const saveAvatar = () => {
        ...
        props.updateGroupAvatar(avatar, props.groupProfileInfo.groupName)
            .then(response => {
                if (response.status === 200) {
                    setSuccess(response.message)
                    props.getGroupProfile(props.groupProfileInfo.groupName)
    ...
    return (
        <div>
            ...
            <SuccessAndErrorAlerts success={success} error={error} groupName={props.groupProfileInfo.groupName}/>

            {props.groupProfileInfo.groupAvatarUrl
                ? <div>
                    <Row style={{marginBottom: "15px"}}>
                        <Col>
                            <div>
                                <Avatar onBeforeFileLoad={onBeforeFileLoad} height={295} onCrop={onCrop}
                                        onClose={onClose} src={config.avatarUrl + props.groupProfileInfo.groupAvatarUrl}
                                        exportAsSquare={true}/>
                            </div>
                        </Col>
                        <Col>
                            <Preview croppedAvatarUrl={props.groupProfileInfo.groupCroppedAvatarUrl} username={props.groupProfileInfo.groupName} preview={preview} />
                        </Col>
                    </Row>
                    <Row className="mb-3">
...
export default GroupAvatarSettings;
